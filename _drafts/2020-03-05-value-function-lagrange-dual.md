...
The optimality condition $$-\nabla f(\bar{x}) \in N_C(\bar{x})$$ is equivalent to the KKT conditions. (The Lagrange multipliers give the representation of $$-\nabla f(\bar{x})$$ in the normal cone.) 
...
Here are some examples of convex conjugates and operations

Duality of properties (unproved)
...
In fact $$\phi = f^{**}$$, the *biconjugate* of f.  Geometrically it is clear that

$$f^{**}(0) = \sup_w -f^*(w) = \phi(0)$$

from the previous discussion, and the same geometry applies at other points.

Clearly $$f^{**} \leq f$$ and the two are equal if $$f$$ is closed and convex.

When $$f(0)$$ is given by an optimization problem, the maximization over hyperplanes that defines $$f^{**}(0)$$ be can viewed a "dual problem."  The familiar Lagrange dual can be view in this way.
